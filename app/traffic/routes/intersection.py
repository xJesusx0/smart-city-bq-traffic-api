...
intersection_router = APIRouter(prefix="/api/intersections", tags=["intersections"])
...
@intersection_router.get(
    "", response_model=list[IntersectionState], dependencies=[Depends(validate_token)]
)
async def get_all_intersections():
    ...
    for key in keys:
        ...
        if data:
            intersections.append(IntersectionState(**json.loads(data)))
