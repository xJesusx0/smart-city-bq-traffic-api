...
from app.core.dependencies import validate_token, GeoInfoServiceDep
...
intersection_router = APIRouter(prefix="/api/geo/intersections", tags=["intersections", "geo"])

@intersection_router.post("/{intersection_id}/heartbeat", response_model=HeartbeatResponse)
...
@intersection_router.get("", response_model=list[IntersectionWithStatus], dependencies=[Depends(validate_token)])
async def get_all_intersections(
    geo_service: GeoInfoServiceDep
):
