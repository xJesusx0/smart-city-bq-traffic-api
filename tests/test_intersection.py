...
from app.core.dependencies import validate_token, get_geo_info_service
import json
...
def test_get_all_intersections_success(authenticated_client):
    with patch("app.traffic.routes.intersection.get_redis_client") as mock_get_redis:
        ...
        mock_data_1 = {
            ...
            "device_name": "esp32-1",
            "ip": "1.1.1.1",
            "semaforo1_verde": 20, "semaforo2_verde": 20, "all_red_time": 2,
            "estado_restante_s": 1, "ciclo_restante_s": 41,
            "next_semaforo1": 20, "next_semaforo2": 20, "next_fetched": False,
            "estado": "S1_VERDE", "last_seen": int(time.time())
        }
        
        mock_redis.get.return_value = json.dumps(mock_data_1)
        
        response = authenticated_client.get("/api/intersections", headers={"Authorization": "Bearer mock-token"})
